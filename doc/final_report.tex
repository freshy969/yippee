...
\begin{tikzpicture}
  \path[mindmap,concept color=darkgray,text=white]
    node[concept] {Yippee Search Module}
    [clockwise from=0]
    child[concept color=teal!] {
      node[concept] {Node 1}
      [clockwise from=90]
      child [concept color=cyan!] { node[concept] {Crawler} }
      child [concept color=purple!] { node[concept] {Indexer} }
      child [concept color=violet!] { node[concept] {Pastry} }
      child [concept color=lime!] { node[concept] {BerkeleyDB} }
    }  
    child[concept color=teal!] {
      node[concept] {Node 2}
      [clockwise from=-10]
    ...
    child[concept color=teal!] { node[concept] {Node 3} }
    child[concept color=magenta!] {
      node[concept] {PageRank}
      [clockwise from=-90]
      child { node[concept] {Hadoop} }
    }
    child[concept color=teal!] { node[concept] {Node 4} };
\end{tikzpicture}
...
The architecture of the Yippee is shown in figure \textbf{Update this graph} 1 \textbf{make this number a reference since there will be more figures this time}.  The 


Section \ref{sec:component} will describe each component and the design decisions leading to its implementation.  Section \ref{sec:evaluation} will present an evaluation for some of the components and 
